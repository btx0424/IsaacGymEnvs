    ...
    def run(self):
        ...
        for iteration in range(self.max_iterations):
            ...
            if iteration % self.log_interval == 0:
                ...
                print(f"iteration: {iteration}/{self.max_iterations}, env steps: {total_env_steps}, episodes: {total_episodes}")
                print(f"runtime: {env_step_time:.2f} (env), {inf_step_time:.2f} (inference), {time.perf_counter()-start:.2f} (total), fps: {total_env_steps/(end-start):.2f}")
                
                for k, v in episode_infos.items():
                    if v:
                        v = torch.cat(v).cpu().numpy().mean(0) # average over episodes
                        train_infos[k] = v
                    print(f"episode {k}: {v}")
