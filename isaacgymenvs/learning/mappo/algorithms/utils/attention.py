...
class CatSelfEmbedding(nn.Module):
    def __init__(self, split_shape, d_model, use_orthogonal=True, activation_id=1):
        ...
        self.split_sizes = [num*dim for num, dim in split_shape]
        self.d_model = d_model
        active_func = [nn.Tanh(), nn.ReLU(), nn.LeakyReLU(), nn.ELU()][activation_id]
        init_method = [nn.init.xavier_uniform_, nn.init.orthogonal_][use_orthogonal]
        gain = nn.init.calculate_gain(['tanh', 'relu', 'leaky_relu', 'leaky_relu'][activation_id])
        ...
        fc = []
        self_dim = split_shape[-1][1]
        for num, dim in split_shape[:-1]:
            fc.append(nn.Sequential(
                init_(nn.Linear(dim+self_dim, d_model)), active_func, nn.LayerNorm(d_model)))
        fc.append(nn.Sequential(
            init_(nn.Linear(self_dim, d_model)), active_func, nn.LayerNorm(d_model)))
        self.fc = nn.ModuleList(fc)

    def forward(self, x, self_idx=-1):
        xs = torch.split_with_sizes(x, self.split_sizes, dim=-1)

        self_x = xs[self_idx] # (bs, self_dim)
        self_dim = self_x.shape[-1]
        result = [] # [(bs, num, dim)]
        for (num, dim), x, fc in zip(self.split_shape[:-1], xs[:-1], self.fc[:-1]):
            # (bs, num, dim) -> (bs*num, dim) -> (bs, num, dim)
            x = torch.cat([x.reshape(-1, dim), self_x.tile(num).reshape(-1, self_dim)], dim=-1)
            result.append(fc(x).reshape(-1, num, self.d_model))
        
        result.append(self.fc[-1](self_x).unsqueeze(1))

        # (bs, sum(num), model_dim)
